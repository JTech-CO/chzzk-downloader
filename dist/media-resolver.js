...
(function () {
  ...
  function metadataStage(url) {
    const u = new URL(url);
    return (u.hostname === 'apis.naver.com' ? 'Neonplayer 재생 정보' : u.pathname.includes('/channels/') ? '목록 조회' : u.pathname.includes('/clip/') ? '클립 정보' : 'VOD 정보') + ' (' + u.hostname + ')';
  }
  async function requestInBackground(url, headers, signal) {
    if (signal?.aborted) throw new DOMException('중지됨', 'AbortError');
    if (!globalThis.chrome?.runtime?.sendMessage) throw new Error(metadataStage(url) + ': 확장프로그램과 치지직 탭을 새로고침해 주세요.');
    const requestId = crypto.randomUUID();
    return new Promise((resolve, reject) => {
      const cleanup = () => signal?.removeEventListener('abort', abort);
      const disconnected = () => { cleanup(); reject(new Error(metadataStage(url) + ': 실행부 연결 실패. 확장프로그램과 치지직 탭을 새로고침해 주세요.')); };
      const abort = () => {
        cleanup();
        try { chrome.runtime.sendMessage({ type: 'CANCEL_METADATA', requestId }).catch(() => {}); } catch (_) { /* Extension may have reloaded. */ }
        reject(new DOMException('중지됨', 'AbortError'));
      };
      signal?.addEventListener('abort', abort, { once: true });
      try {
        chrome.runtime.sendMessage({ type: 'FETCH_METADATA', requestId, url, accept: headers.Accept || '' }).then(response => {
          cleanup();
          if (signal?.aborted) return;
          if (!response || response.error || typeof response.text !== 'string') reject(new Error(response?.error || metadataStage(url) + ': 영상 정보 재요청에 응답이 없습니다. 확장프로그램을 새로고침해 주세요.'));
  ...
  async function request(url, format, headers, signal) {
    ...
    try {
      ...
      if (signal?.aborted) throw new DOMException('중지됨', 'AbortError');
      if (timedOut) throw new Error(metadataStage(url) + ': 영상 정보 요청 시간이 초과됐습니다. 다시 시도해 주세요.');
      if (error.name === 'TypeError') {
        clearTimeout(timer);
        log('[연결] ' + metadataStage(url) + ': 화면 요청 실패, 확장프로그램에서 다시 요청합니다.');
        const text = await requestInBackground(url, headers, signal);
        return format === 'json' ? JSON.parse(text) : text;
      }
      throw new Error(metadataStage(url) + ': ' + error.message);
  ...
  async function callNeonplayer(videoId, inKey, signal) {
    ...
    log('[NEO] 재생 정보 요청 (apis.naver.com)');
