...
network-extension.test.js maps the two official API hostnames and a synthetic
media host to a local HTTPS server only inside isolated Chromium. Playback
metadata intentionally has no CORS headers. The test grants local-network-access
only in its isolated profile so loopback permission checks cannot mask CORS. The preserved v2.3.2 fails before
starting; the current extension must save inKey VOD/clip and rewind HLS files.
A disconnected MP4 Range probe must fall back to exactly one native download.
Saved hashes and HLS duration patching are checked. Minimal MP4 boxes are
synthetic transport fixtures, not a codec-playback or production VOD test.
