...
test('OPFS streaming writes out-of-order downloads in byte order', async () => {
  context.__delivered = false;
  vm.runInContext(`
    fetchSeg = async function (segment) {
      const index = Number(segment);
      for (let i = 0; i < 4 - index; i++) await Promise.resolve();
      return Uint8Array.of(index).buffer;
    };
    deliverDownload = async function () {
      __delivered = true;
    };
  `, context);
  ...
  const stream = {
    ...
    fh: {},
    writable: {
      write: async (buffer) => { written.push(...new Uint8Array(buffer)); },
  ...
  await invoke(
    ...
    stream,
    null,
    { concurrency: 4, expectedTotalBytes: 4, byteProgress: true }
  );

  assert.deepEqual(written, [0, 1, 2, 3]);
  assert.equal(closed, true);
  assert.equal(context.__delivered, true);
});

test('OPFS streaming deletes an incomplete file on size mismatch', async () => {
  vm.runInContext(`
    fetchSeg = async function () {
      return Uint8Array.of(1).buffer;
    };
    deliverDownload = async function () {
      throw new Error('incomplete file must not be delivered');
    };
  `, context);
  ...
  let closed = false;
  const stream = {
    root: { removeEntry: async () => { removed = true; } },
    name: 'test.mp4',
    fh: {},
    writable: {
      write: async () => {},
  ...
  await assert.rejects(
    () => invoke(
      ...
      stream,
      null,
      { concurrency: 2, expectedTotalBytes: 3, byteProgress: true }
    ),
    /완성 파일 크기 불일치/
  );
