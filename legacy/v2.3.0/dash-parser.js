...
(function () {
  ...
  function parse(xml, url) {
    ...
    if (sets.some(set => isAudio(set) || children(set, 'Representation').some(isAudio))) throw new Error('별도 오디오 트랙을 사용하는 DASH는 아직 지원하지 않습니다.');
    const candidates = [];
    for (const set of sets) {
      ...
      for (const rep of children(set, 'Representation')) {
        if ((rep.getAttribute('mimeType') || '').startsWith('audio')) continue;
