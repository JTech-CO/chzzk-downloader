// Chzzk Downloader v2.3.0 - progressive catalog and keyed download views.
(function () {
  'use strict';
  ...
  const labels = { queued: '대기 중', info: '정보 확인 중', downloading: '다운로드 중', merging: '마무리 중', saving: '파일 저장 중', stopping: '중지 중', paused: '일시 중지', cancelled: '중지됨', interrupted: '중단됨', error: '오류', done: '저장 완료' };
  const icons = {
  ...
  panel.innerHTML = `
    <section class="cdl-shell" id="cdl-shell" role="dialog" aria-label="Chzzk Downloader" hidden>
      ...
      <footer class="cdl-footer"><span>v2.3.0</span><div><button id="cdl-copy-log" class="cdl-text-button" hidden>로그 복사</button><button id="cdl-log-toggle" class="cdl-text-button" aria-expanded="false">진단 로그</button></div></footer>
  ...
  let routeKey = '', scanToken = 0, scanAbort, windowKey = '', frame = 0, dirtyCatalog = true, dirtyJobList = true, logText = '', polling = false;
  ...
  function updateCard(card) {
    const job = jobs.get(idFor(card.item)); card.action.disabled = Boolean(job && ACTIVE.has(job.status));
    text(card.action, job && ACTIVE.has(job.status) ? labels[job.status] : job?.status === 'done' ? '다시 받기' : '다운로드');
  ...
  function updateJobRow(row, job) {
    ...
    row.retry.hidden = active || job.status === 'done'; text(row.retry, job.hasLocalFile ? '다시 저장' : job.resumable ? '이어받기' : '다시 시도');
  ...
  async function syncJobs() {
    ...
    try {
      const result = await send({ type: 'GET_JOBS' }), known = new Set(result.jobs.map(job => job.id));
    ...
    catch (error) { log(error.message); } finally { polling = false; }
  }
  ...
  async function startDownload(item) {
    ...
    try {
      ...
      const response = await send({ ...common, ...payload }); receiveJob(response.job);
