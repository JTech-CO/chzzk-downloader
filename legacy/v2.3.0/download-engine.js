...
async function fetchSeg(segment, signal, creds, attempts = 4, onBytes) {
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    try {
      ...
      throwIfAborted(signal);
      if (!isRetryableBodyError(error)) { if (result) await cancelResponseBody(result.response); throw error; }
  ...
  throw new Error(`세그먼트 ${attempts}회 실패: ${safeError(lastError)}`);
}
async function probeDirectRange(url, signal) {
  ...
  try {
    ...
    if (response.status !== 206 || !range || range.start !== 0 || range.end !== 0) {
      await cancelResponseBody(response);
      if (response.status === 429 || response.status >= 500) throw new Error(`사전 확인 HTTP ${response.status}`);
...
async function removeCheckpoint(id) {
  ...
  if (checkpoint && validFileName(checkpoint.name)) {
    const root = await navigator.storage.getDirectory();
    try { await root.removeEntry(checkpoint.name); } catch (error) { if (error.name !== 'NotFoundError') throw error; }
  }
...
async function ensureSpace(remainingBytes) {
  if (!navigator.storage.estimate || !remainingBytes) return;
  const { quota, usage } = await navigator.storage.estimate();
  if (Number.isFinite(quota) && Number.isFinite(usage) && quota - usage < remainingBytes + 64 * MiB) {
    throw new Error('브라우저 임시 저장 공간이 부족합니다. 완료·중지된 작업을 정리하거나 디스크 공간을 확보해 주세요.');
  }
}
async function directDownload(job) {
  ...
  throwIfAborted(signal);
  if (!probe.supported || probe.totalBytes < DIRECT_RANGE_MIN_BYTES) {
    job.mode = 'native';
  ...
  if (CdlCore.resumableCheckpoint(checkpoint, probe, DIRECT_RANGE_CHUNK_BYTES) && validFileName(checkpoint.name)
    ...
    } catch (_) { file?.close(); file = null; completed.clear(); }
  }
  if (!file) {
  ...
  try {
    await ensureSpace(probe.totalBytes - completedBytes());
    await save();
...
async function downloadStreaming(job, segments, creds, finalizer = null) {
  ...
  try {
    ...
    file.close(); await removeCheckpoint(job.id).catch(() => {}); throw error;
...
async function fetchHlsText(url, signal, creds) {
  ...
  const bytes = await fetchSeg(assertSafeHttpsUrl(url, 'HLS 재생목록 URL'), signal, creds);
...
async function execute(job) {
  try {
    ...
    event(job, { status: cancelled ? (resumable ? 'paused' : 'cancelled') : 'error',
      message: cancelled ? (resumable ? '중지됨 · 받은 구간에서 이어받을 수 있습니다.' : '작업을 중지했습니다.') : safeError(error),
