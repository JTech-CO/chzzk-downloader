...
(async () => {
  ...
  try {
    ...
    for (const [target, folder] of [[oldPage, 'legacy/v2.3.1'], [page, 'src']]) {
    ...
    const report = { checkedAt: new Date().toISOString(), channel, baseline: '2.3.1', current: '2.3.2', results, scope: 'Public live API metadata and two 64 KiB Range samples; no complete production VOD download or speed measurement.' };
