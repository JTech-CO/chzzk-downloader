...
async function mount({ archived = false, xml = mpd, status = 200, content = {} } = {}) {
  ...
  const source = archived ? path.join(root, 'legacy/v2.3.1') : path.join(root, 'src');
...
test('reproduction: v2.3.1 rejects muxed VOD with optional audio and then selects a neighbouring preview', async () => {
