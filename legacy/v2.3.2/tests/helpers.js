...
function harness({ entry = 'download-engine.js', storage = memoryStorage(), smallRanges = false, chrome: chromeOverrides, timers, sourceDirectory = path.join(root, 'src') } = {}) {
  const events = [], logs = [], rpcs = [], calls = [], loaded = new Set();
  ...
  function load(file) {
    ...
    loaded.add(file);
    let source = fs.readFileSync(path.join(file.startsWith('legacy/') ? root : sourceDirectory, file), 'utf8');
  ...
  return { context, invoke, storage, events, logs, rpcs, calls, downloads, chrome, listeners,
